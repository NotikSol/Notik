...
mysql_select_db("db_test",$db)	
	or die("<br>Ошибка выбора БД!".mysql_error()."</p>");		
			$sql=/*$this->conn->prepare*/("INSERT INTO web (login, sname, name, mname, ps, email, phone, age, sex, my_country)
										  VALUES (?,?,?,?,?,?,?,?,?,?)");
			//prepare - Подготавливает запрос к выполнению и возвращает ассоциированный с этим запросом объект
			/*$query->bind_param("sssssssdss", $this->login, $this->sname, $this->name, $this->mname, $this->ps, $this->email, $this->phone, $this->age, $this->sex, $this->my_country);
			 //sssssssdss  - определяет типы данных для каждого параметра по порядку
	...
	/*		$query = sprintf($this->sql, mysql_real_escape_string($this->login),
						   mysql_real_escape_string($this->sname), 
						   mysql_real_escape_string($this->name), 
						   mysql_real_escape_string($this->mname), 
						   mysql_real_escape_string($this->ps), 
						   mysql_real_escape_string($this->email), 
						   mysql_real_escape_string($this->phone),
						   mysql_real_escape_string($this->age),
						   mysql_real_escape_string($this->sex),
						   mysql_real_escape_string($this->my_country));
	echo "<br>".$query."<br>";
$result = mysql_query($query)
	or die('<br>В данный момент нельзя добавить пользователя:<br>');
mysql_close($db);*/
